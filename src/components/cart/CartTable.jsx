...
function CartTable({ cartItems, setCartItems }) {
  // Obtener cartCounter desde el Context usando desestructuración de objeto
  ...
  // Maneja el cambio de cantidad para un item del carrito
  const handleQuantityChange = (event, id) => {
    ...
    if (quantity !== "") {
      ...
      if (updatedCart[index].discounted_price) {
        price = updatedCart[index].discounted_price * event.target.value;
      } else {
    ...
    // Remueve el item si la cantidad es 0
    if (quantity === "0") {
